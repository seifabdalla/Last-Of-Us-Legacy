...
public class Game {
	...
	public static void adjustVisibility(Hero h) {
		...
		/*Point pn= Main.gploc(p);
		int x=pn.x;
		int y=pn.y;
		Main.checkrec(Main.getgg().getChildren(),x,y);*/
		for (int i = -1; i <= 1; i++) {
			int cx = p.x + i;
			if (cx >= 0 && cx <= 14) {
				for (int j = -1; j <= 1; j++) {
					...
					if (cy >= 0 && cy <= 14) {
						if (cy >= 0 && cy <= map.length - 1) {
							...
							Point pg=Main.gploc(new Point(cx,cy));
							int l=pg.x;
							int m=pg.y;
							Main.checkrec(Main.getgg().getChildren(),l,m);
	...
	public static void spawnNewZombie() {
		...
		Image l=new Image(Main.class.getResourceAsStream("Zomf.png"));
	    ImageView v= new ImageView(l);
	    v.setFitHeight(50);
	    v.setFitWidth(50);
	    Main.gg.setHalignment(v, HPos.CENTER);
		Main.gg.setValignment(v, VPos.CENTER);
	    z.setiv(v);
	    Point p=Main.gploc(new Point(x,y));
	    int gx=p.x;
	    int gy=p.y;
	    Main.gg.add(z.getiv(),gx,gy);
	    Main.hide(gx,gy);
	}
